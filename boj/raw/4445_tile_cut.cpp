...
int main() {
    ...
    while (true) {
        ...
        do {
            grid.push_back("");
            getline(cin, grid.back());
        } while (!grid.back().empty());
        grid.pop_back();
        // cout << grid.size() << endl;
        // for (auto &r : grid) cout << r << endl;
        if (grid.empty()) break;

        // k paths of W I N
        // need to ensure nodes only used once so use split trick
        // serialization:
        // in_rc = (n * cols + c) * 2
        // out_rc = (n * cols + c) * 2 + 1
        // src = rows * cols * 2
        ...
        for (int r = 0; r < rows; ++r) {
            for (int c = 0; c < cols; ++c) {
                if (grid[r][c] == 'W') f.addEdge(src, (r * cols + c) * 2, 1);
                if (grid[r][c] == 'N') f.addEdge((r * cols + c) * 2 + 1, dst, 1);
                f.addEdge((r * cols + c) * 2, (r * cols + c) * 2 + 1, 1);
                auto check = [&](int nr, int nc) {
                    if ((grid[r][c] == 'W' && grid[nr][nc] == 'I') || (grid[r][c] == 'I' && grid[nr][nc] == 'N')) {
                        f.addEdge((r * cols + c) * 2 + 1, (nr * cols + nc) * 2, 1);
                    }
                };
                if (r) check(r-1, c);
                if (c) check(r, c-1);
