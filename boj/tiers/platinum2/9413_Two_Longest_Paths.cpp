...
void solve() {
    int n, m; cin >> n >> m;
    // to find max length of 2 separate paths as long as possible, convert to flow graph
    // src -(cap=2,cost=0)-> entry -(cap=1,cost=-1)
    // to enforce each node being in 1 path split node i into in_i, out_i
    // in_i -(cap=1,cost=-1)-> out_i
    // entry -(cap=1,cost=0)-> in_i (represents starting path at node i)
    // edge (i, j) in original graph: out_i -(cap=1, cost=0)-> in_j
    // out_i -(cap=1,cost=0)->dst (represents ending path at node i)
    // -cost = combined length of paths
    // flow = number of paths (should be 2)
    // serialization:
    // in_i = i*2
    // out_i = i*2+1
    // src = n*2
    ...
    for (int i = 0; i < n; ++i) {
        ...
        f.addEdge(i*2, i*2+1, 1, -1);
    }
    for (int i = 0; i < m; ++i) {
        int a, b; cin >> a >> b; --a; --b;
        f.addEdge(a*2+1, b*2, 1, 0);
    }
